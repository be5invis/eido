...
var createFnThunk = require('./processor').createFnThunk;
...
scope.lambda = lazy(function(_args, definition){
	...
	var e = definition.scope || this;

	var initParameters = function(s, args) {
		for(var i = 0; i < parameterNames.length; i++){
			if(!functionIsLazy || parameterIsLazy[i]) {
				s[parameterNames[i]] = args[i]
			} else {
				s[parameterNames[i]] = s.force(args[i])
			}
		}		
	}
	var fCStatic = function(){
		var s = fork(e);
		initParameters(s, arguments);
	...
	// For functions defined by \lambda, its apply property takes 3 arguments:
	// thisp, args and scopeOverride. Once the 3rd argument is given, the function
	// will be call in "modifer mode", which means that the upvalue of this call
	// will be set to the scopeOverride argument.
...
// NOTE: I've removed these functions.
// Please use \define
/*
...
scope.getDefScope = function(f){
	return ((f instanceof Function && f.scope) ? f.scope : this)
}
...
scope.array = function(){return SLICE.call(arguments, 0)}
scope.array['new'] = scope.newArray = function(n){return new Array(n)};
scope.array.join = function(arr){return arr.join.apply(arr, SLICE.call(arguments, 1))};
scope.array.slice = function(arr){return arr.slice.apply(arr, SLICE.call(arguments, 1))};
scope.array.map = function(arr){return arr.map.apply(arr, SLICE.call(arguments, 1))};
scope.toString = function(x){return x + ''};
...
scope.lambdaDSL = function(f, ports){
	var s = fork(this.getDefScope(f));
	ports = ports || {};
	for(var property in ports) if(HAS(ports, property)){
		s[property] = ports[property];
	};
	return createFnThunk({apply: function(t, a, o){
		if(o){
			var sd = fork(o);
			for(var property in ports) if(HAS(ports, property)){
				sd[property] = ports[property];
			};
			return f.apply(sd, a, sd);
		} else {
			return f.apply(s, a)
		}
	}}, s);
