...
scope.def_ = lazy(function(name, definition){
	this[this.call(name)] = definition;
	return '';
});
...
scope.lambda = lazy(function(_args, definition){
	...
	for(var i = 0; i < arguments.length - 1; i += 1){
		arglist[i] = this.call(arguments[i]);
	};
...
scope.defun = lazy(function(){
	return this.def(this.call(arguments[0]), this.lambda.apply(this, SLICE.call(arguments, 1)))
});
scope.defun_ = lazy(function(){
	return this.def(this.call(arguments[0]), lazy(this.lambda.apply(this, SLICE.call(arguments, 1))))
});
...
scope['if'] = lazy(function(condition, thenPart, elsePart){
	if(this.call(condition))
		return this.cmod(thenPart);
	else if(elsePart)
		return this.cmod(elsePart);
	else return;
})

scope['while'] = lazy(function(condition, body){
	while(this.call(condition))
		this.call(body);
})
