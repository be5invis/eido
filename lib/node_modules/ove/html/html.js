...
exports.apply = function(){
	...
	scope.html__lit = safe_tags_replace;
	scope.html_p = function(s){
		if(s){
			return new Tag('p', s)
		} else {
			return ''
	...
	scope.html_ul = function(){
		var s = '';
		for(var i = 0; i < arguments.length; i++)
			s += arguments[i]
		return new Tag('ul', s)
	};
	scope.html_ol = function(){
		var s = '';
		for(var i = 0; i < arguments.length; i++)
			s += arguments[i]
		return new Tag('ol', s)
	};
	scope.html_li = function(s){
		return new Tag('li', s)
	};
	scope['html_inline_`'] = function(s, num){
	...
	var htmlOverrides = function(){
		var s = {};
		s.__lit = scope.html__lit;
		s._p = scope.html_p;
		s._ul = scope.html_ul;
		s._ol = scope.html_ol;
		s._li = scope.html_li;
		s['inline_`'] = scope['html_inline_`'];
		s['inline_*'] = scope['html_inline_*'];
		s['inline_~'] = scope['html_inline_~'];
		return s;
	}();
	scope.__htmlBlock = function(block){
		return this.lambdaDSL(block, htmlOverrides);
	};
	scope.htmlSegment = scope.__lazy(function(block){
		return (scope.__htmlBlock(block))()
	});
	...
	scope.table = scope.__lazy(function(h, b){
		if(!b) { b = h; h = '' }
	...
	scope['withID'] = function(cls, element) {
		if(element instanceof Tag) {
			var _ = Object.create(element);
			_.props = _.props + ' id="' + cls + '"'
			return _
		} else {
			return element
		}
